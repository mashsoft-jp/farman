...
// ── ダイアログの Alt ショートカット規約 ─────────────────────────
//
// 同じ機能には同じキーを割り当てる。ダイアログを渡り歩いても指が迷わない
// ようにするため。新しいダイアログを作るときはここに合わせること。
//
//   共通アクション
//     O  OK / 実行            X  キャンセル
//     C  閉じる               A  適用
//     G  移動 (Go to)         Y / N  はい / いいえ
//
//   繰り返し出る項目
//     N  名前                 D  ディレクトリ (場所を選ぶ行)
//     P  パス                 F  ファイルのみ
//     I  ディレクトリのみ     H  隠しファイル
//     S  サブディレクトリ     Z  サイズ
//     M  更新日時             T  内容 / テキスト
//     R  正規表現             C  大文字小文字を区別 (閉じるが無いダイアログ)
//
// そのダイアログにしか出てこない項目は、ラベルに含まれる文字から選ぶ
// (含まれない文字を指定すると Windows / Linux で "ラベル (&K)" と末尾に
// 付いてしまい、収まりが悪いため)。
//
// 割り当てる範囲は「フォームの各行」「ON/OFF のチェック」「ダイアログの
// ボタン」まで。1 つの行の中に並ぶ従属的な入力欄 (Min / Max / From / To /
// 単位など) には付けない。行を選んだ直後に Tab で辿れるうえ、字面の候補も
// 尽きるため。
//
