...
変更点」だけを要約する役割を担う。英語版は [CHANGELOG.en.md](CHANGELOG.en.md) にある。
...
- **アーカイブ**
  ...
  - **作成オプション**: 圧縮レベル (0〜9)、zip の **パスワード暗号化**
    (AES-256 固定、パスワード確認欄 + 一致検証付き)。暗号化非対応の
    libarchive ビルドでは平文を作らずエラーにする
...
v0.9.6 からの主なユーザー向け変更点。コミット / PR 単位の詳細は
[GitHub Releases](https://github.com/mashsoft-jp/farman/releases/tag/v0.9.7) を参照。

### Added

- **統合プロパティダイアログ**: 名前・場所・サイズ・更新日時・作成日時などを
  1 つのダイアログに集約。更新日時・属性 (パーミッション) をその場で編集でき、
  所有者 / グループは名前と ID を表示 (選択ファイルで共通のときだけ名前を変更可、
  Windows では非表示)。複数選択時は合計サイズ / 件数を集計表示
- **「ビュアーで開く...」をプラグインから自動生成**: 登録済みプラグインからメニューを
  生成し、これまで一覧に出なかった動画 / 音声ビュアーなども選べる
- **外部ビュアーウィンドウにステータスバー**: 内部表示と同じ情報を表示。動画は
  フォーマット / コーデック / 解像度 / 再生時間の要約を表示
- **画像 / 動画ビュアーのズームを強化**: 動画にもズーム率と「ウィンドウに合わせる」を
  追加。外部ビュアーは W キーで現在のズームに応じた実サイズにウィンドウを調整。
  画像ビュアーにキーボードショートカットを追加 (`I` 情報 / `Space` 再生・停止 /
  `R` 回転 / `F` ウィンドウに合わせる / `T` 透過切替 / `+`・`-` 25% 刻みズーム /
  `W` ウィンドウを画像に合わせる)
- **ビュアーの表示設定を拡充**: CSV / TSV に表示フォント設定、Markdown に表示フォントと
  本文 / 背景 / リンク色の設定 (Light / Dark 別に指定可、未指定はテーマ既定色に追従)
- **進捗ダイアログにファイル単位のバイト進捗バー**: 大きいファイルのコピー / 移動中に
  現在ファイルの転送量を表示

### Changed

- **ビュアー設定を各プラグイン設定に一本化**: 対象拡張子 / MIME・フォント・配色・画像の
  透過を「外観」「プラグイン」で分散管理していたのを、各ビュアーのプラグイン設定へ集約。
  フォント / 配色は設定内の Light / Dark 切替でテーマ別に編集可能
- **ダークモードのテーマ追従を改善**: ツールバーやペイン下部のソート・フィルタ表示が
  明るいまま残る問題を解消
- **macOS の既定フォントを Helvetica 12pt に統一** (UI・ビュアー共通)
- **組み込みツールメニューの項目名を UI 言語にローカライズ**
- **ネットワーク / リムーバブルドライブではゴミ箱を無効化**し、完全削除のみを提示
- 確認ダイアログから「?」アイコンを削除

### Fixed

- macOS の Helvetica で、ファイルリストやプロパティの日時などが 2 行に折り返される問題
- Markdown で生 HTML を含む文書が途中で切れて表示される問題
- 長いファイル名のときに削除確認ダイアログのレイアウトが崩れる問題
- 起動時の初期ディレクトリが存在しない場合にホーム / ルートへフォールバックしない問題
- 0 バイトのファイルがバイナリビュアーで「読み込み中」のまま止まる問題
- UTF-8 フラグの無い日本語 (Shift-JIS) zip でアーカイブ内のファイル名が文字化けする問題
- PDF を外部ビュアーで開いて閉じるときにクラッシュすることがある問題
- 背景でのディレクトリ更新中にリネームするとクラッシュ (use-after-free) する問題
- Windows でマップされたネットワークドライブのゴミ箱可否を誤判定する問題
