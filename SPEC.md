...
- 既に保存されているディレクトリでは、設定済みのチェック状態が初期値となる。
  ...
  デフォルトに戻す。
- ダイアログの初期値は、保存済み上書きがあればその内容。無ければ **いま表示に
  効いているソート・フィルタ** (チェックを外して一時的に適用したものを含む)。
  一時的に変えた直後に開き直しても、表示とダイアログの内容が食い違わない。
- 開いた時点の内容が Behavior カテゴリのデフォルトと違う場合は、パス見出しの下に
  その旨と違う項目 (ダイアログ内のラベルと同じ名前) を表示する。保存済み上書きか
  一時的な設定 (未保存) かも区別する。デフォルトと同じなら何も出さない。
- パネルフッターのソート・フィルタ表示の先頭に、いま効いている設定の出どころを出す:
  **[カスタム]** (保存済み上書きと同じ) / **[デフォルト]** (上書きが無く、
  Behavior カテゴリのデフォルトと同じ) / **[一時的]** (それ以外。保存せずに OK した
  ダイアログの内容や見出しクリックの並べ替えなど)。ソート部にはドットファイルの扱いを
  ON / OFF とも出す (「ドット優先」/「ドット無視」)。
- ディレクトリに移動した際、保存済み上書きがあれば自動で適用する。
  なければパネルの既定（Behavior カテゴリのデフォルト）を適用する (一時的な設定は
  そのディレクトリを離れると消える)。
...
- ログペインはクリックでフォーカスを当てられ、マウスで文字列を選択できる (読み取り専用)。
  フォーカスがあるときは **`Ctrl+C` で選択範囲をクリップボードへコピー**、**`Ctrl+A` で
  ログ全体を選択** する (選択が無ければ `Ctrl+C` は何もしない)。どちらもファイル
  マネージャー側のコマンド (パスのコピー / すべて選択) に割り当てられたキーだが、ログに
  フォーカスがある間はログ側が優先する (`LogPane` が `ShortcutOverride` で受け取る)。
  右クリックメニューの「コピー」「すべて選択」も使える。
...
- **転送確認ダイアログ**: コピー元、ファイル一覧、コピー先、上書きモード、
  ...
  - フォルダ参照ボタンは Tab で到達したことが視認できるよう、
    ...
    弱いことの補正)。
- 上書き時の個別確認ダイアログ (上書きモード `Ask` のとき、競合ごとに表示):
  - 操作は 上書き (Alt+W) / 新しい名前でリネーム (Alt+R、初期選択) /
    このファイルをスキップ (Alt+S) の 3 択。スキップはそのエントリだけ飛ばして
    続行し、Cancel (Alt+X) は操作全体を中止する。
  - **「以降の重複にもこの選択を適用」チェックボックス** (Alt+A): ON で OK すると、
    同じ操作の残りの競合ではダイアログを出さずに同じ選択を使う。
    リネームを選んでいた場合、2 件目以降は自動リネームの suffix テンプレートで
    一意な名前を付ける (入力した名前は 1 件目にしか使えないため)。
    その操作限りの記憶で、Settings には保存しない。
  - 進捗の件数はスキップしたファイルも「処理済み」に数え、100% で完了する。
...
- **パスワード付きアーカイブ**は対応済み (ZIP / 7z / RAR、libarchive がサポート
  ...
  - 間違いなら再プロンプト、規定回数失敗で中止 → 通常 FS に戻る
...
- **ネスト**: アーカイブ内のアーカイブも Enter で更に潜れる (v1.0.0 で対応)。
  - パスは `!` 区切りを多段にする (`a.zip!/d/inner.zip!/sub`)。区切りは最も後ろの
    `!` から「左側がアーカイブ名として成立するか」で判定するので、エントリ名や
    ディレクトリ名に `!` を含んでいても取り違えない
  - libarchive はアーカイブの中のアーカイブを直接読めないため、内側は一時
    ディレクトリへ書き出してから開く。`ArchiveContext` は論理パス
    (`archivePath`) と実体のパス (`readPath()`) を別に持ち、表示・ナビゲーションは
    論理パス、libarchive へ渡すのは実体を使う
  - 展開結果は `NestedArchive` が論理パス → 実ファイルで覚えるので、入り直しの
    たびに外側を開き直さない。大元のアーカイブが更新されたときだけ展開し直す。
    一時ディレクトリはセッション限りで、終了時に丸ごと消える
  - 段ごとに暗号化されていれば段ごとにパスワードを訊く
  - 段数の上限は設定 → アーカイブ「ネスト段数上限」(既定 = 無制限)
  - 一時展開先は設定 → アーカイブ「一時ディレクトリ」(既定 = システムの
    一時ディレクトリ)。設定変更は次回起動から効く
...
- **MIDI ビュアー / プレイヤー** (`.mid` / `.midi`) *(後日 or プラグインビュアー対応)*
  - SMF (Standard MIDI File) の再生。動画 / 音声ビュアー (MediaView) に組み込むか、
    専用の MIDI プレイヤーを外部プラグインとして別途作成するかは未定。
  - 再生には MIDI シンセ (サウンドフォント等) が必要で外部依存が大きいため、
    外部プラグイン寄りが妥当か要検討。
...
4. **ビュアー固有設定の置き場を整理する**。現状フォント / 配色 / 対象拡張子 /
   MIME は Settings → Appearance の各ビュアーサブタブにあり、対象拡張子は
   Settings → Plugins の詳細とも重複している。これらビュアー属性を Plugins 詳細側へ
   集約し、拡張子の二重管理を解消する (UI 再配置・設定移行・テストに波及)。
5. **ビュアーの対象拡張子マッチングを設定駆動にする**。現状 `supportedExtensions()` /
   `supportedMimeTypes()` がコード固定で glob (パターン) マッチャが無く、設定での
   拡張子増減が `canHandle` 判定に完全には反映されない。glob マッチャを導入し、
   プラグインの拡張子設定を実際のオープン判定へ効かせる (4 と関連)。
...
WCX 風の具体例として **lzh** (古く日本独自の形式) の展開対応は、本体には組み込まず
`IArchivePlugin` で外部対応する候補。**展開のみとし、圧縮は行わない**方針。

### プラグインのインストール *(v1.1.0)*

外部プラグイン (`.dylib` / `.dll` / `.so`) の導入・更新・削除を、プラグイン
ディレクトリへの手作業コピー無しにアプリ内で完結させる。導入経路は 2 つ:

1. **ファイルからの導入** (ドラッグ＆ドロップ / ファイル選択)。非公式プラグインも対象。
2. **公式プラグイン一覧からの導入 / 更新** (GitHub Releases から取得)。

どちらも「**検証 → 退避 (pending) → 次回起動時に反映**」の共通基盤
(`PluginInstaller`) を通る。実装順は 共通基盤 → 1 → 2。

**共通基盤: 検証 (ロードしない)**

導入時はプラグインを **dlopen しない** (= 第三者コードを実行しない)。
`QPluginLoader::metaData()` はライブラリをロードせずバイナリ内の Qt メタデータを
読むだけなので、これで次の順に判定する。1 つでも外れたら導入せず理由を表示する。

1. 拡張子が実行 OS の共有ライブラリか (macOS `.dylib` / Windows `.dll` /
   Linux `.so`)。他 OS 用は「この OS 用のプラグインではありません」。
2. メタデータの `IID` が `com.farman.IViewerPlugin/` または
   `com.farman.IArchivePlugin/` で始まるか。始まらなければ「farman のプラグインでは
   ありません」。
3. `IID` が本体の `FarmanIViewerPlugin_iid` / `FarmanIArchivePlugin_iid` と完全一致
   するか。不一致は「この farman には対応していません (インターフェース版数の
   不一致)」として、プラグイン側と本体側の版数を併記する。
4. メタデータ `MetaData.MinHostVersion` が本体バージョン以下か
   (`hostSatisfiesMinVersion`、起動時ロードと同じ判定)。
5. 配置先は `IID` の種別から自動決定する (ビュアー → `viewers/`、アーカイブ →
   `archives/`)。ユーザーには選ばせない。どの種別のタブにドロップしても同じ。

pluginId / 名前 / 版数 / 制作者はロードしないと取れないため、導入時点では
**ファイル名の基底名**で同一性を判定する。配布物のファイル名は版数つき
(`<Name>-vX.Y.Z-<os>-<arch>.<ext>`、Web サイトのダウンロードリンクもこの名前に依存) で、
この命名は変えない。基底名は、拡張子・版数以降 (`-vX.Y.Z...`)・ブラウザが付ける
重複番号 (` (1)`) を落として小文字化したもの (`PluginInstaller::pluginBaseName`。
`ModelViewerPlugin-v0.3.0-macos-arm64.dylib` と `ModelViewerPlugin.dylib` は同じ
プラグイン)。実際のロードと `qobject_cast` 検証
(priority 値域・`author()` 必須など) は再起動後の通常ロード経路に任せ、失敗すれば
既存の一覧 / 詳細ダイアログのエラー表示で確認できる。

**共通基盤: 退避と反映**

ロード中のライブラリは Windows では上書き・削除できず、macOS / Linux でも
dlopen 中の差し替えは避けたい。そこで導入・更新・削除はすべて退避しておき、
**次回起動時のプラグイン読込前**に反映する。`QPluginLoader::unload()` による
ホットスワップは、ビュアーが開いている可能性があり危険なので行わない。

- 導入 / 更新: 検証済みファイルを `<プラグインディレクトリ>/pending/viewers/` または
  `pending/archives/` へ、ファイル名は変えずにコピーする。同じプラグイン (基底名が同じ)
  の導入待ちがあれば置き換える。
- **別名の古い版が導入済みの場合は、その削除を一緒に退避する** (`removals.json` に
  `{"path": 古いファイル, "replacedBy": 導入待ちファイル}` として記録)。残すと同じ
  pluginId のプラグインが 2 つになり、ファイル名順で先に読まれる古い版が有効になって
  新しい版は「重複 ID」として弾かれてしまう。導入を取り消せばこの削除も取り消され、
  置き換えるファイルが pending から失われていた場合は起動時にも古い版を消さない。
- 削除: `<プラグインディレクトリ>/pending/removals.json` に、プラグインディレクトリ
  からの相対パス (`viewers/Foo.dylib`) を記録する。対象は `viewers/` / `archives/`
  直下の外部プラグインだけで、同梱プラグインやディレクトリ外のファイルは削除できない。
- 起動時 (`main.cpp`、`ViewerDispatcher::loadPlugins` より前) に
  `PluginInstaller::applyPending()` を呼び、(1) removals.json の各ファイルを削除、
  (2) pending 配下の各ファイルを本来の場所へ移動 (同名は上書き)、の順に反映して
  pending を空にする。同じファイルに削除と導入の両方が退避されていれば導入が勝つ
  (後から行った操作で取り消せるよう、導入の退避時に同名の削除記録を消し、
  削除の退避時に同名の pending ファイルを消す)。
- 反映に失敗したもの (権限など) は pending に残し、次回起動で再試行する。
- 導入 / 更新 / 削除の退避と、起動時の反映結果は、いずれもログペイン (`Logger`) に残す。
- プラグインディレクトリが書込不可の場合は、退避の時点でエラーとして案内する。

**Settings → Plugins ページ (導入と管理の窓口)**

外部プラグインの導入・更新・アンインストールは、ビュアー / アーカイブの種別を問わず
**Settings → Plugins** ページ 1 箇所で行う。導入後の設定 (有効 / 無効・ファイル
パターン・プラグイン固有の設定) は従来どおり Settings → Viewer / Archive が持ち、
同梱プラグインもそちらにだけ出る。ページの構成 (上から順):

- **「外部プラグインの読込みを許可する」チェック** と注意書き。外部プラグイン全体に
  関わるスイッチなので、グループに入れずページの一番上に置く。
- **「更新を確認」ボタン** と確認状況のラベル。公式プラグインの最新版を取得する
  (下記「公式プラグインの更新とインストール」)。
- **外部プラグインの一覧** (全種別)。列は 状態 / 名前 / 種別 / バージョン / 最新版 /
  詳細... / 更新する / アンインストール...。並ぶのは、導入済みの外部プラグイン (ロード
  失敗・ブロック中のものも含む)、導入待ちの新規ファイル、そして **まだ導入していない
  公式プラグイン** (マニフェストにあって導入されていないもの。状態は「-」= 未インストール)。
  公式プラグイン専用のダイアログは設けず、この一覧で済ませる。
  - 状態: ✅ 読み込み済み / 🔒 ブロック中 / 🚫 無効 / ❌ 失敗 (理由はツールチップ) /
    ⏳ 再起動後にインストール・更新・アンインストール / - 未インストール。
  - **「詳細...」** (全行): プラグインの詳細ダイアログを開く。名前 / 種別 / 区分 (公式・
    非公式) / 状態 / バージョン / 制作者・URL / プラグイン ID / パス / エラー全文に加え、
    公式プラグインは 最新版 / 更新の状況 / 説明 / 必要な farman / リリースノートへの
    リンクも出す。読み取り専用 (設定の変更は Viewer / Archive ページ)。一覧の Enter /
    Space / ダブルクリックでも開く。
  - **更新があるときの見せ方**: 「最新版」列を「❗ <版>」と太字 + 橙色で表示し
    (ツールチップは「更新があります: <導入済みの版> → <最新版>」)、その行の「更新する」
    ボタンも太字にする。印は赤い絵文字の ❗ にする: 青地の 🆕 は選択行の青い背景に
    埋もれ、文字の「NEW」は選択中に他の列と同じ白になって目立たないため。「更新を確認」の
    横のラベルの先頭にも「❗ n 件の更新があります。」を太字 + 橙色で出す (無ければ
    「更新はありません。」をグレーで)。
  - **「更新する」** (公式プラグインの行だけ。未導入の行ではラベルが「インストール」):
    押せるのは、更新がある / 未導入 / 導入済みだがロードされておらず版数が不明 (入れ直し)
    のとき。最新です・未確認・確認できない・この環境向けの配布物が無い・farman が古い・
    プラグインディレクトリの外に導入済み・退避中、のときは無効表示で、理由をツールチップ
    (と「最新版」列のツールチップ、詳細ダイアログ) に出す。非公式の行にはボタンを出さない。
  - **「アンインストール...」** (導入済みの行。確認あり): 退避中の行は「取り消し」になり、
    導入 / 更新 / 削除の退避を取り消せる。プラグインディレクトリの外にある外部プラグイン
    (同梱ディレクトリに置かれた第三者製など) は消せないので無効表示。未導入の公式
    プラグインの行には出さない。
  - キーボード: 行移動は ↑/↓。**Tab は選択している行のボタンだけを辿る** (一覧 →
    その行の 詳細... → 更新する → アンインストール... → 「ファイルからインストール...」。
    Shift+Tab は逆順で一覧へ戻る。無効なボタンは飛ばす)。フォーカスチェーン任せだと
    どの行を選んでいても 1 行目のボタンへ飛んでしまうため、明示的に制御する。ボタンに
    フォーカスが来たらその行を選択行にする。ボタン上の Enter / Space はそのボタンを押す
    (Enter がダイアログの OK に届いて閉じてしまわないようにする)。押した後は同じ位置の
    ボタンへフォーカスを戻す。
- **進行表示**: 更新の確認中とダウンロード中だけ、一覧の下にメッセージと進捗バーを出す。
- **ドロップ領域** (点線の角丸枠。Web のファイルアップロード欄と同じ見せ方)。枠の中に
  「ここにプラグインファイル (.dylib) をドロップしてインストール」/「または」/
  **「ファイルからインストール...」ボタン** を縦に並べ、ドラッグ＆ドロップでも導入できる
  ことを見て分かるようにする。ドロップ自体はページのどこに落としても受け (複数可)、
  ファイルをページ上にドラッグしている間は枠をハイライト色 + 薄い塗りで強調する。
  枠はパレットの色で描くので Light / Dark の切り替えに追従する。ドロップ元は Finder / エクスプローラー等
  (設定ダイアログはモーダルなので、開いている間は farman 自身のファイル一覧からは
  ドラッグできない)。
- **再起動バナー**。退避中の変更が 1 件でもあると「プラグインの変更は farman の
  再起動時に反映されます」と **「farman を再起動」ボタン** を出す。押すと確認なしで、
  ダイアログの設定を OK と同じく保存してから再起動する (導入フローで ON にした
  「外部プラグインの読込みを許可」もここで保存される)。
- **プラグインディレクトリ** (ページの一番下。めったに変えないため)。
  「外部プラグインの読込みを許可する」とともに v1.0.x では Settings → General に
  あったものを移設した。どちらも、変更して OK / 適用すると従来どおり再起動するかを
  確認する。

**ファイルからの導入の流れ**

- ハッシュ検証ができず制作者も分からない (ロードしないと `author()` を読めない)
  ので、導入前に「入手元を信頼できるプラグインだけを導入してください」の確認を出す。
  制作者は再起動後に一覧で確認できる。
- 同名のファイルが導入済み / pending のものは、導入の確認に「導入済みのファイルを
  置き換えます」と添える (この確認は導入全体で 1 回)。
- **別のバージョンと見られるプラグイン (基底名が同じで別名) が導入済みの場合は、
  個別に尋ねる**: 「<新ファイル名> の別のバージョンと見られるプラグインが既に
  インストールされています (<旧ファイル名>、ロード済みなら版数も)。インストール済みの
  方をアンインストールして <新ファイル名> をインストールしますか?」。Yes (既定) なら
  古い版の削除を一緒に退避して導入する。No ならそのファイルは**導入しない**
  (両方置くと重複 ID で片方しか読み込まれないため、併存はさせない)。
  一覧では古い版の行が「⏳ 再起動後に更新 (<新ファイル名> に置き換え)」になり、
  導入待ちファイルは別の行としては出さない。
- 「外部プラグインの読込みを許可する」が OFF のときは、導入しても動かないので、
  ON にするかを尋ねる (黙って ON にはしない)。Yes ならページのチェックを入れる
  (保存は OK / 適用 / 再起動のとき)。断った場合も導入自体は続行でき、再起動後は
  「ブロック中」として出る。
- 成功した導入は一覧の ⏳ 行と再起動バナーで示し、完了ダイアログは出さない。検証 /
  退避に失敗したファイルがあるときだけ、ファイル名 + 理由をまとめて表示する。
- **macOS の隔離属性**: ブラウザ経由で入手したファイルには `com.apple.quarantine`
  が付き、未署名 / 未公証のプラグインは dlopen が Gatekeeper に拒否される。コピー時に
  属性を剥がすことは**しない** (ユーザーのセキュリティ判断を迂回しない)。代わりに、
  外部プラグインのロード失敗時は dlopen のメッセージから原因を推定し
  (`classifyPluginLoadError`)、エラー表示に案内を添える: Gatekeeper による拒否
  (署名 / 公証 / 隔離属性) と、依存ライブラリが見つからない場合 (配布版 farman 向けに
  ビルドされたプラグインを、別の Qt を使う開発ビルドで読もうとした場合など)。

**公式プラグインの更新とインストール**

公式プラグインも Settings → Plugins の一覧で扱う (専用のダイアログは設けない)。
どのプラグインが公式かは同梱のマニフェストで分かるので、一覧には最初から未導入の公式
プラグインも並ぶ。最新版の確認 (ネットワークアクセス) は次のときだけ行う:

- Plugins ページを **初めて表示したとき** (1 時間キャッシュ。Settings を開いただけでは
  確認しない)。本体の「起動時にアップデートを確認」を OFF にしている場合は自動では
  確認せず、「更新を確認」を押すよう案内する。
- **「更新を確認」ボタン** を押したとき (キャッシュを無視して取り直す)。

確認結果はボタンの横に出す (最終確認の日時 / オンラインの一覧を取得できず同梱の一覧を
使っている / 一部のリリース情報を取得できなかった)。

- **一覧のソース (マニフェスト)**: `docs/plugins/manifest.json`。GitHub Pages で
  `https://farman.mashsoft.co.jp/plugins/manifest.json` として配信され、アプリはこれを
  取りに行く。取得できないとき (オフライン / サイトに未反映 / `formatVersion` が未対応)
  は、ビルド時点の内容を同梱した `:/plugins/manifest.json` にフォールバックし、その旨を
  ダイアログに表示する。1 件の項目は `id` (= `pluginId()`) / `repo`
  (`owner/farman-plugin-<name>`) / `kind` (`viewer` | `archive`) / `fileName` (配布
  ファイル名の基底。導入済みファイルとの突き合わせに使う) / `minFarmanVersion` /
  `name`・`description` (`ja` / `en`)。必須項目の欠けた項目・未知の種別・`owner/repo`
  形式でない repo は無視する。Web サイトのプラグインページ (`docs/plugins/index.html`)
  は説明・スクリーンショット・更新履歴を手書きで持つので、**公式プラグインを増やす
  ときはページのカードとマニフェストの両方に足す**。
- **最新リリース**: 各プラグインについて GitHub Releases API
  (`/repos/<repo>/releases/latest`) から取り、このプラットフォーム用のアセット
  (`<fileName>-vX.Y.Z-{macos-arm64|windows-x64|linux-x86_64}.{dylib|dll|so}`) と同名の
  `.sha256` を選ぶ。**`.sha256` の無い配布物は導入対象にしない**。ダウンロード元は
  `https://github.com/<repo>/releases/download/` 配下に限る。
- **キャッシュ**: 未認証 API の上限 (60 req/h) に配慮し、結果を 1 時間キャッシュする
  (プロセス内 + キャッシュディレクトリの `plugins/catalog.json`。導入 → 再起動 →
  開き直し、を繰り返しても API を叩き直さない)。取得に失敗した項目を含む結果は
  キャッシュから使わない。「更新を確認」はキャッシュを無視する。
- **導入済みかどうかの判定**: 種別が同じで、ロード済みなら `pluginId` がマニフェストの
  `id` と一致、そうでなければファイルの基底名 (`pluginBaseName`) が `fileName` と一致。
  更新の有無は導入済みの `version()` と最新リリースのタグの semver 比較。
- **更新 / インストールの流れ**: 確認は 1 回 (「<名前> を <版> に更新しますか?」/
  「<名前> <版> をインストールしますか?」。導入済みの別名ファイルがあれば「インストール
  済みのファイルはアンインストールされます: ...」を添える。入手元の確認は SHA256 照合で
  代えるので求めない) → `.sha256` を取得 → 本体を `.part` にダウンロード (進捗バー) →
  **SHA256 を照合** (不一致・取得失敗はエラーにしてファイルを残さない) → ファイルからの
  導入と同じ検証 (IID / `MinHostVersion`。種別がマニフェストと違えば拒否) → 退避
  (`stageInstall`)。一覧の行が ⏳ になり再起動バナーが出る。外部プラグインの読込みが
  OFF なら ON にするかを尋ねる。ダウンロード中に設定ダイアログを閉じたら中断する。
- 公式配布物は CI で署名 + 公証済みなので、macOS の隔離属性の問題は起きない。
  なお配布物は配布版 farman の Qt に合わせてビルドされているため、別の Qt を使う
  開発ビルドでは導入できても読み込めない (依存ライブラリ不足の案内が出る)。
...
- ソート設定
  ...
  - ドットファイルを先頭に（`Sort dot files first`）
    - ON: `.` で始まるものを、ディレクトリ / ファイルそれぞれのグループ内で先頭にまとめる。
    - OFF: 名前で比べるとき、先頭の `.` 1 つを除いた名前で比べて名前の並びに混ぜる
      (`.bashrc` は `b` の位置)。除いた名前が同じ (`.bashrc` と `bashrc`) ならドット無しが先。
      `..` (親ディレクトリ) は設定に関わらず常に先頭。
...
操作に関する TIPS (ショートカット / 隠れがちな機能の小ネタ) を、起動時に 1 件ずつ
ダイアログで見せる。

### 表示のタイミング

- **1 日 1 回、その日の最初の起動時** に表示する。1 日の区切りはローカル時刻の 0 時。
  最後に表示した日付 (`behavior.tipsLastShownDate`、`yyyy-MM-dd`) を Settings に持ち、
  今日と一致すれば表示しない。日付を跨いで起動したままの場合、次に起動するまで出ない。
- 設定 → 全般 → 起動設定の **「起動時に TIPS を表示する」** (`behavior.showTipsOnStartup`、
  既定 ON) が OFF なら表示しない。
- 起動時の順序は What's New (アップデート直後のみ) → TIPS → 自動アップデートチェック
  (1500ms 遅延)。**同じ起動で What's New が出た回は TIPS を出さない** (ダイアログの連続
  表示を避ける)。このとき日付は記録せず、その日の次の起動で出る。
- ヘルプメニューの **「TIPS を表示...」** (`help.tips`、既定キー無し) でいつでも開ける
  (起動時の表示を OFF にしていても)。メニューでは「キーバインド一覧」の次、
  「プラグイン...」の上に置く (どちらも使い方を知るための項目)。手動で開いた場合も日付を記録する。

### ダイアログ

- 本文は What's New と同じ方式で、**言語別 Markdown をリソースに同梱** する
  (`:/tips/tips_<lang>.md`。`## ` 見出し 1 つ = TIPS 1 件で、見出しがタイトル、続く
  本文が内容。言語は Settings::language に従い、無ければ `en` にフォールバック)。
  表示前に `MarkdownSanitize` を通す (生 `<` 対策。What's New と同じ穴を作らない)。
  タイトルの先頭に **「(<番号> / <全体数>)」** を添える (ファイル内の並び順の番号)。
- **開いたときに出す 1 件はランダム** (`QRandomGenerator::global()`。起動時の自動表示も
  ヘルプからの表示も同じ)。**「前へ」「次へ」はランダムではなく、ファイルの並び順で
  前後の TIPS** に進む。最後の TIPS で「次へ」を押すと 1 件目に、1 件目で「前へ」を
  押すと最後の TIPS に回る。
- ボタン: **「前へ」** (Alt+P)、**「次へ」** (Alt+N) (どちらも Tab でフォーカスを当てて
  Enter でも押せる) と **「閉じる」** (Alt+C、Enter / Esc)。「前へ」「次へ」は既定
  ボタンにしない (Enter は「閉じる」)。TIPS が 1 件しかなければ「前へ」「次へ」は無効。
- 左下に **「起動時に TIPS を表示する」チェックボックス** (Alt+S)。設定の同名項目と
  同じ値で、切り替えるとその場で Settings に保存する (設定ダイアログの一般タブを
  開き直せば反映されている)。
- モーダル。サイズは 520×360 程度で、本文が長ければスクロール。

### TIPS 本文の管理

- `resources/tips/tips_ja.md` / `tips_en.md`。両言語で同じ件数・同じ順序に揃える
  (番号で対応づけはしないが、取りこぼしに気付きやすくするため)。
- 新機能を足したら、必要に応じて TIPS も足す (What's New とは別。TIPS は「知らないと
  使わない機能」を短く紹介するもの)。
- 本文でキーに触れるときは、キーそのものではなく **`{key:<command id>}`** と書く
  (例 `{key:file.copy}`)。表示時に `TipsDialog::expandKeys` が、そのコマンドに今
  割り当てられているキー (先頭の 1 つ、キーバインド一覧と同じネイティブ表記。
  コード表記 + 太字 + 青系の色で目立たせる) に置き換えるので、ユーザーがキーバインドを変えていても説明が実際のキーと一致する。
  Qt の Markdown 読み込みはコード表記の太字を落とすため、表示後に `emphasizeKeys` が
  等幅の部分へ太字と色 (ダーク `#6CB4FF` / ライト `#0A58CA`、本文の背景の明暗で選ぶ) を付け直す。
  本文のコード表記はキー専用 (ほかの用途に使わない)。本文フォントは標準の 1.25 倍、見出しは 1.4 倍。
  割り当てが無ければ「(未割り当て)」。コマンドに紐付かないキー (`Shift`+文字の頭文字
  ジャンプ、`Esc`) だけはそのまま書く。
...
## スライドショー *（未実装 / v0.9.8 候補）*

複数選択したファイル / ディレクトリを、ビュアーで順次自動表示する機能。

### 起動と対象

- 起動: ファイル一覧で複数選択した状態から、コマンド `view.slideshow`
  （メニュー / ショートカット / ツールバー）で開始。
- 開始時に**開始ダイアログ**を出し、以下を指定する（各初期値は設定画面で設定）:
  - **表示間隔 (秒)** — 静止画等の送り時間。設定既定は **10 秒**。
  - **サブフォルダの再帰有無 / 深さ** — 選択ディレクトリを展開する際の再帰。
  - **スライドショー全体のループ有無** — 最終ファイル後に先頭へ戻るか。
- 対象の展開:
  - 選択にディレクトリが含まれる場合、その中のファイルを列挙して順序に含める。
    **再帰有無 / 深さは開始ダイアログの指定**に従う（既定値は設定画面）。
  - 並び順は一覧の現在のソート順に従う。
  - ビュアーで開けない種別（対応プラグイン無し）は**スキップ**する想定
    （バイナリビュアーへ強制フォールバックはしない）。※要確定。

### 表示と送り

- 動画 / **音声** / アニメ画像（GIF・WebP 等）:
  - **強制的に自動再生**する（メディアビュアーの autoplay 設定が OFF でも自動再生。
    ループは OFF に上書き）。音声も動画と同様に扱う。
  - **再生完了直後に次のファイルへ**進む。アニメ画像は 1 ループ完了で送る
    （QMovie のフレーム周回検知が必要）。
  - 実装: `MediaView` に「再生完了」通知、`ImageView` に「アニメ 1 周完了」
    通知を用意し、スライドショー制御がそれを受けて次へ。
- それ以外（静止画 / テキスト / PDF / CSV 等）:
  - 開始時に決めた**一定時間表示してから次へ**（タイマー送り）。

### 表示面

- 表示面は farman の**ビュアー表示モード設定（インライン / 外部ウィンドウ）に準拠**する。
  スライドショー専用の表示面は設けず、通常のビュアーと同じ場所でファイルを差し替える。

### 操作 / 終了

- スライドショー中の操作: 一時停止 / 再開、前 / 次、停止（Esc）。※UI 詳細は実装時に確定。
- 最終ファイル到達で停止。**ループ有無は開始ダイアログの指定**に従う（既定値は設定画面）。

### 関連コンポーネント / 実装メモ

- 新規 `SlideshowController`（選択展開・順序管理・送りタイマー・完了通知の集約）。
- 表示面は既存のビュアー表示モード（インライン `ViewerPanel` / 外部ウィンドウ）を
  そのまま再利用し、ファイルを差し替える。
- 新規コマンド `view.slideshow`。設定キー（案）:
  `slideshowDefaultSeconds` (既定 10) / `slideshowDefaultRecurse` (再帰有無) /
  `slideshowDefaultRecurseDepth` (深さ) / `slideshowDefaultLoop` (ループ有無)。
- メディアの autoplay / loop 上書き、アニメ画像の loop-once セマンティクスが要点。

### 未確定事項

- 開始ダイアログ / スライドショー中の操作 UI の詳細。
- 再帰深さの表現（無制限 / N 段）。
- ビュアーで開けない種別のスキップ挙動の確定。

---

## ディレクトリサイズのバックグラウンド算出 *（未実装 / v0.9.8 候補）*

ファイル一覧で、ディレクトリの合計サイズをバックグラウンド計算して
「サイズ」列に表示する。

### 表示仕様

- 現状 `FileListModel`（列: `Name` / `Type` / `Size` / `LastModified`）では
  ディレクトリの `Size` 列に `"<DIR>"` を表示している
  （`src/model/FileListModel.cpp:633`）。これを変更する:
  - **`Size` 列にはバックグラウンド計算した合計バイト数**を表示。
  - **`Type`（種別）列に `"<DIR>"` を表示**する（表記は現状の `"<DIR>"` を踏襲）。
- 算出を行わない条件:
  - 一覧に **`Size` 列が無い**（Columns 設定で非表示）場合は計算しない。
  - 設定「**ディレクトリのサイズ計算を行わない**」が ON の場合は計算しない。
  - これらの場合、`Size` 列は空欄（または従来どおりの表記）にする。

### 計算 / キャッシュ

- 表示中ディレクトリのサブディレクトリについて、バックグラウンドで再帰的に
  合計サイズを算出（`PropertiesWorker` の再帰サイズ集計ロジックを再利用 / 転用）。
  一覧のディレクトリ移動でキャンセルする。
- 算出タイミングを設定で選択:
  - **毎回算出**（一覧表示のたびに再計算）
  - **キャッシュ**（一度算出したら一定時間はキャッシュ値を使う）
    - **キャッシュ保持時間**も設定画面で指定。
- **強制再算出**: キャッシュ有効時でも、ディレクトリサイズを強制的に再計算する
  **ボタン + ショートカットキー**を用意する（現在の一覧のキャッシュを無効化して再計算）。
- キャッシュキーは絶対パス（+ mtime を併用して無効化するかは**実装開始時に決める**）。

### 関連コンポーネント / 実装メモ

- 新規バックグラウンドワーカー（`ThumbnailWorker` と同様に非同期→モデル更新）。
- 計算結果は `FileListModel` の該当行 `Size` 列へ反映（`dataChanged`）。
- `Size` によるソートは、未算出のディレクトリを 0 / 不明として扱う。
- 設定キー（案）: `computeDirectorySizes`(bool) / `directorySizeCacheMode`
  (`always` / `cached`) / `directorySizeCacheSeconds`(int)。
- 強制再算出用の新規コマンド（案）`file.recompute_dir_sizes`（ボタン + ショートカット）。
- `Type` 列のディレクトリ表記は現状どおり `"<DIR>"`。
...
- **アーカイブ作成オプション** — `CreateArchiveDialog` に作成オプションを実装済み。
  ...
  - **パス文字コード** (Shift_JIS / UTF-8 など) — Windows 旧 zip 互換。
    **読み取り側は v1.0.0 で対応**: 設定 → アーカイブの形式ごとの詳細で
    「ファイル名の文字コード」を選べる (既定は自動判別 = UTF-8 として妥当なら
    UTF-8、でなければ Shift_JIS)。指定は一覧 / 展開 / サムネイルの全経路で効く。
    作成側 (`zip:hdrcharset`) は引き続き見送り。
