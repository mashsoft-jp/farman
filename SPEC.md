...
- **開いたときに出す 1 件はランダム** (`QRandomGenerator::global()`。起動時の自動表示も
  ヘルプからの表示も同じ)。**「次の TIPS」はランダムではなく、ファイルの並び順で
  次の TIPS** に進み、最後の TIPS で押すと 1 件目に戻る。
- ボタン: **「次の TIPS」** (Alt+N。Tab でフォーカスを当てて Enter でも押せる) と
  **「閉じる」** (Alt+C、Enter / Esc)。「次の TIPS」は既定ボタンにしない (Enter は
  「閉じる」)。
