...
**ファイル名**でしか同一性を判定しない。実際のロードと `qobject_cast` 検証
...
- 導入 / 更新: 検証済みファイルを `<プラグインディレクトリ>/pending/viewers/` または
  `pending/archives/` へコピーする。同名ファイルが既に pending にあれば置き換える。
...
- ハッシュ検証ができず制作者も分からない (ロードしないと `author()` を読めない)
  ...
  制作者は再起動後に一覧で確認できる。
- 同名ファイルが導入済み (または pending) のものは、導入の確認に「導入済みの
  ファイルを置き換える」旨を添える (確認は導入全体で 1 回)。
...
- **macOS の隔離属性**: ブラウザ経由で入手したファイルには `com.apple.quarantine`
  ...
  外部プラグインのロード失敗時のエラー表示に「署名 / 隔離属性が原因の可能性」を
  案内する一文を足す (macOS のみ)。

**公式プラグイン一覧からの導入 / 更新** (GitHub Releases から取得)。
...
**ファイルからの導入 (ドラッグ＆ドロップ / ファイル選択)**

- 受け口は Settings → Viewer / Settings → Archive の「Installed Plugins」一覧。
  ファイル (複数可) をドロップするか、一覧下の「ファイルからインストール...」
  ボタンでファイルを選ぶ (キーボードだけで操作できる経路)。ドロップ元は Finder /
  エクスプローラー等 (設定ダイアログはモーダルなので、開いている間は farman 自身の
  ファイル一覧からはドラッグできない)。
- ハッシュ検証ができず制作者も分からない (ロードしないと `author()` を読めない)
  ので、導入前に「入手元を信頼できるプラグインだけを導入してください」の確認を出す。
  制作者は再起動後に一覧 / 詳細ダイアログで確認できる。
- 同名ファイルが導入済み (または pending) のものは、導入の確認に「導入済みの
  ファイルを置き換える」旨を添える (確認は導入全体で 1 回)。
- 「外部プラグインの読込みを許可する」が OFF のときは、導入しても動かないので、
  導入の確認と合わせて ON にするかを尋ねる (黙って ON にはしない)。断った場合も
  導入自体は続行でき、一覧には従来どおり「ブロック中」として出る。
- 結果は 1 件ずつ集計して最後にまとめて表示する (成功 n 件 / 失敗はファイル名 +
  理由)。成功時は「再起動後に有効になります」と明示する。
- 退避中の変更は、一覧の下の「farman の再起動後に反映」欄に 1 件ずつ出し
  (導入 / アンインストール + ファイル名 + 種別)、各行の「取り消し」で退避を取り消せる。
  Viewer / Archive どちらのタブの欄にも全種別の退避状況を出す。導入済みプラグインの
  更新待ち / 削除待ちは、一覧の状態列にも ⏳ で示す。
- アンインストールは、プラグインディレクトリ配下にある外部プラグインの詳細ダイアログの
  「アンインストール...」ボタンから行う (確認あり)。退避中は同じボタンが
  「アンインストールの取り消し」になる。
- **macOS の隔離属性**: ブラウザ経由で入手したファイルには `com.apple.quarantine`
  が付き、未署名 / 未公証のプラグインは dlopen が Gatekeeper に拒否される。コピー時に
  属性を剥がすことは**しない** (ユーザーのセキュリティ判断を迂回しない)。代わりに、
