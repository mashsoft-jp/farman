...
**公式プラグイン一覧からの導入 / 更新** *(ファイルからの導入の後に実装)*

- 公式一覧のソースは Web サイトと同じ GitHub Pages に置くマニフェスト
  `docs/plugins/manifest.json` (id / repo / 種別 / 表示名・説明 (ja, en) /
  最低 farman 版)。Web サイトのプラグインページもこれを読む (一覧の二重管理を避ける)。
- 各プラグインの最新版・アセット URL・`.sha256` は GitHub Releases API から取る。
  アセット命名規則は Web サイトと同じ
  `<Name>-vX.Y.Z-{macos-arm64|windows-x64|linux-x86_64}.{dylib|dll|so}`。
  未認証 API の上限 (60 req/h) に配慮し、取得結果は 1 時間キャッシュし、
  一覧を開いたときだけ取りに行く。
- ダウンロードと SHA256 照合は本体アップデート (`UpdateDownloader`) と同じ手順。
  照合に通ったファイルを上記の共通基盤 (検証 → 退避) に渡す。公式配布物は CI で
  署名 + 公証済みなので隔離属性の問題は起きない。
- 一覧の 1 行 = 1 プラグイン: 名前 / 種別 / 最新版 / 導入済み版 / 状態 (未導入・
  導入済み・更新あり) / 操作。マニフェストに無い導入済み外部プラグインは「非公式」
  として扱い、操作は従来どおり (有効 / 無効・アンインストール) のみ。
- 更新判定は導入済み `version()` と最新リリースタグの semver 比較。
