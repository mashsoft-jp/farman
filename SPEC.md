...
- **外部プラグインの一覧** (全種別)。列は 状態 / 名前 / 種別 / バージョン / 最新版 /
  ...
  - **「詳細...」** (全行): プラグインの詳細ダイアログを開く。名前 / 種別 / 区分 (公式・
    ...
    Space / ダブルクリックでも開く。
  - **更新があるときの見せ方**: 「最新版」列を「NEW <版>」と太字 + 橙色で表示し
    (ツールチップは「更新があります: <導入済みの版> → <最新版>」)、その行の「更新する」
    ボタンも太字にする。絵文字の 🆕 (青地) は選択行の青い背景に埋もれるので使わない。
    文字なら選択中はハイライト用の文字色で描かれ、どちらの状態でも読める。「更新を確認」の
    横のラベルの先頭にも「n 件の更新があります。」を同じ太字 + 橙色で出す (無ければ
    「更新はありません。」をグレーで)。
