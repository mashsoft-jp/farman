...
- **外部プラグインの一覧** (全種別)。列は 状態 / 名前 / 種別 / バージョン / 制作者 /
  操作。対象は外部プラグインだけ (ロード失敗・ブロック中のものも含む)。導入待ちの
  新規ファイルも、ファイル名の行として並べる。
  - 状態: ✅ 読み込み済み / 🔒 ブロック中 / 🚫 無効 / ❌ 失敗 (理由はツールチップ) /
    ⏳ 再起動後にインストール・更新・アンインストール。
  - 操作ボタン: 通常は「アンインストール...」(確認あり)。退避中の行は「取り消し」に
    なり、導入 / 更新 / 削除の退避を取り消せる。プラグインディレクトリの外にある外部
    プラグイン (同梱ディレクトリに置かれた第三者製など) は消せないので無効表示。
  - キーボード: 一覧の Enter / Space で選択行の操作ボタンを押す。操作ボタン自体にも
    Tab でフォーカスが当たる (一覧 → 各行のボタンを上から順 → 「ファイルから
    インストール...」)。ボタン上の Enter / Space はそのボタンを押す (Enter が
    ダイアログの OK に届いて閉じてしまわないようにする)。押した後は同じ行のボタンへ
    フォーカスを戻す。
...
Settings → Plugins の **「公式プラグインを入手...」** ボタンで「公式プラグイン」
ダイアログ (`PluginCatalogDialog`) を開く。ネットワークに出るのはこのダイアログを
開いたとき (と「再読み込み」) だけで、起動時の自動チェックはしない。
...
- **キャッシュ**: 未認証 API の上限 (60 req/h) に配慮し、結果を 1 時間キャッシュする
  ...
  キャッシュから使わない。「再読み込み」はキャッシュを無視する。
- **一覧**: 名前 / 種別 / 最新版 / インストール済み (版数。ロードされておらず不明なら
  「あり」) / 状態 / 操作。選択行の説明・リリースノートへのリンク・導入済みファイル名を
  一覧の下に出す。状態と操作:
  - 未インストール → **インストール**
  - 更新あり (導入済みの `version()` < 最新版。semver 比較) → **更新**
  - インストール済み (バージョン不明。ブロック中 / ロード失敗 / 無効) → **再インストール**
  - 最新です / プラグインディレクトリの外にインストール済み (同じ ID が 2 つになるので
    ここからは入れさせない) / この環境向けの配布物がありません / farman X 以降が必要 /
    リリース情報を取得できません → 操作なし
  - 退避中 (⏳ 再起動後にインストール / 更新 / アンインストール) → 操作なし
    (取り消しは Plugins ページの一覧で行う)
  - 導入済みかどうかは、種別フォルダ内のファイルの基底名 (`pluginBaseName`) と
    マニフェストの `fileName` の一致で判定し、版数はロード済みレコードの `pluginId`
    一致で取る。キーボード操作は Plugins ページの一覧と同じ (Enter / Space、Tab で
    各行のボタン)。
- **導入の流れ**: 確認は 1 回 (「<名前> <版> をインストールしますか?」。導入済みの
  別名ファイルがあれば「インストール済みのファイルはアンインストールされます: ...」を
  添える。入手元の確認は SHA256 照合で代えるので求めない) → `.sha256` を取得 →
  本体を `.part` にダウンロード (進捗バー) → **SHA256 を照合** (不一致・取得失敗は
  エラーにしてファイルを残さない) → ファイルからの導入と同じ検証
  (IID / `MinHostVersion`。種別がマニフェストと違えば拒否) → 退避 (`stageInstall`)。
  ダイアログを閉じると Plugins ページの一覧と再起動バナーが更新され、外部プラグインの
  読込みが OFF なら ON にするかを尋ねる。ダウンロード中に閉じたら中断する。
