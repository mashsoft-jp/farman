...
**farman 1.1.0 以降はアプリ内で導入できる**: 設定 → プラグイン に `.dylib` / `.so` /
`.dll` をドラッグ＆ドロップするか、「ファイルからインストール...」で選ぶ。種別
(ビュアー / アーカイブ) は IID から自動判定され、farman の再起動時に配置される
(同じページの「farman を再起動」ボタンですぐ反映できる)。アンインストールも同じページの
一覧から行う。導入後の有効 / 無効や設定は 設定 → ビュアー / アーカイブ で行う。
導入時はライブラリをロードせず、Qt メタデータ (IID / `MinHostVersion`) だけで適合を
判定するので、**`Q_PLUGIN_METADATA` の IID が本体と一致していないと導入できない**。

手作業で導入する場合は、ビルドした `.dylib` / `.so` / `.dll` を farman の外部プラグイン
ディレクトリ配下の種別サブフォルダに置き、farman を再起動する。**外部プラグインの読込みは既定でオフ**なので、
