...
**farman 1.1.0 以降はアプリ内で導入できる**: 設定 → ビュアー (または アーカイブ) の
プラグイン一覧に `.dylib` / `.so` / `.dll` をドラッグ＆ドロップするか、一覧下の
「ファイルからインストール...」で選ぶ。種別 (ビュアー / アーカイブ) は IID から自動判定
され、farman の再起動時に配置される。アンインストールは各プラグインの「詳細...」から。
